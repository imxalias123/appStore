...
class AppStore extends Component {
  state = {
    activeTabs: tabsList[0].tabId,
    searchInput: '',
  }

  updateActiveTab = tabId => {
    this.setState({activeTabs: tabId})
  }

  getFilteredApps = () => {
    const {activeTabs} = this.state
    const filteredApps = appsList.filter(each => each.category === activeTabs)
    return filteredApps
  }
  ...
  render() {
    ...
    const searchResult = this.getSearchResult(filteredApps)
        ...
        <ul className="tabList-container">
          {tabsList.map(each => (
            <TabItem
              ...
              isActive={activeTabs === each.tabId}
            />
          ))}
        </ul>
